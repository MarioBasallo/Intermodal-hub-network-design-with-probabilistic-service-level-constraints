...
void modelM2(void) {
	...
	fprintf(CutInfo, "a1\ta2\tCut intercept with vertical axis\tDerivative of tilde{f}^[ka]\n");
...
mycutcallback(CPXCENVptr env,
	...
	*useraction_p = CPX_CALLBACK_DEFAULT;
	...
	//printf("Objective value at incumbent solution: %.0f \n\n", objval);
	...
	for (int a = 0; a < svars; a++) {
		...
		if ((x[FeasHubs.pos[a1]] > 0.5) && (x[FeasHubs.pos[a2]] > 0.5)) {
			...
			//for (int com = arc1[a1].arc2[a2].dim - 1; com >= 0; com--) {
			//	k = arc1[a1].arc2[a2].com[com];
			//	akpos = Ak[k].node[a1].node2[a2];
			//	k = arc1[a1].arc2[a2].pos2[ypos[akpos]];

			for (int k = arc1[a1].arc2[a2].dimys - 1; k >= 0; k--) {

				if (x[arc1[a1].arc2[a2].pos[k] + FeasHubs.dim + xvars] > 0.5) {
					//if ((x[Ak[arc1[a1].arc2[a2].com[com]].node[a1].node2[a2] + FeasHubs.dim] > pow(10, -6))) {
